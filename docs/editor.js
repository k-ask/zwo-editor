...
function getZoneLabel(power) {
    for (let i = 0; i < ZONES.length; i++) {
        if (power < ZONES[i].limit) return `ZONE ${i + 1}`;
    }
...
function updateSegment(id, field, value) {
    ...
    if (s) {
        value = toHalfWidth(value); // Sanitize full-width chars
            ...
            <div class="segment-content-wrapper">
                <div class="segment-header">
                    <span class="segment-type" style="color: white; text-shadow: 0 1px 2px rgba(0,0,0,0.5);">
                        ${s.type === 'SteadyState' ? getZoneLabel(s.power) : s.type}
                    </span>
